...
def leer_inbound():
    ...
    trenes = []
    c = 0
    with open("input3dias.csv", "r") as file:
        for line in file:
            if c == 0:
                pass
            else:
...
class Simulacion:
    ...
    def __init__(self, dia, estacion, inbound, t_inicial, horizonte):
        ...
        self.proximo = datetime.strptime(t_inicial, "%H:%M")+ timedelta(minutes=60)
    ...
    def knapsack(self, ingresar_tiempo):
        ...
        for track1 in self.estacion.cla_tracks:
            if len(track1.vagones) != 0:
                for track2 in self.estacion.cla_tracks:
                    if len(track2.vagones) != 0:
...
# outbound = [["AD", "AF"],["AF", "AW", "AY"],["AH", "AK"],["AN", "AP", "AJ", "BG"],["AR", "AW", "AY"]]
            ...
            if tracks.largo == 2:
                for track in self.estacion.cla_tracks:
                    if track == tracks.track1 or track == tracks.track2:
                        vagones = []
                        for vagon in track.vagones:
                            vagon.salida = self.tiempo_simulacion + timedelta(minutes=self.estacion.multi_pull)
    ...
    def run(self):
            ...
            while self.tiempo_simulacion < self.horizonte:
                ...
                # Si comentas esta linea puedes ver los problemas de cuello de botella que hubo en el hump :)
            ...
            print("N trenes que no entraron: {}\n".format(self.estacion.sobra))
            print("Pulls: {}\n".format(self.pulls))
            print("Multipulls: {}\n".format(self.multipulls))
...
estacion = Estacion(10, 185, 42, 60, 7, 207, 2, 0, 10, 15, 150, 25)
